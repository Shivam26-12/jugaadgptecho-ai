"""
===================================================================
 ECHOSCOPE - Main AI Brain Software (v3.32 - Audio Boost Fix)
===================================================================
This script is the complete "brain" for your project.

BUG FIXES (v3.32):
- WAKE WORD SENSITIVITY: The problem is not the 'sensitivity'
  setting, but the *volume* of the audio.
- THE FIX: Added a numpy-based AMPLIFICATION step inside
  the 'run_wake_word_detector' thread (line 389).
- The audio is now boosted by 2x *before* being
  fed to the AI, making it much more sensitive.
- (Retains all other fixes: Mic ID 18, Resampling,
  Stereo-to-Mono, Audio Priority, etc.)
"""
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                # --- SENSITIVITY FIX v3.32: Amplify the audio signal ---
                # We will boost the volume of the audio before sending it to the AI.
                BOOST_FACTOR = 2.0 
                
                # Convert to a larger integer type for safe multiplication
                audio_data_boosted = audio_data_16k.astype(np.int32) * BOOST_FACTOR
                
                # "Clip" the audio to prevent it from exceeding the 16-bit limit
                # (which is -32768 to 32767)
                audio_data_clipped = np.clip(audio_data_boosted, -32768, 32767).astype(np.int16)
                
                # Feed the *boosted, resampled, mono, 16kHz* audio to Porcupine
                keyword_index = porcupine.process(audio_data_clipped)
                # --- End v3.32 Fix ---
...
def main():
    ...
    try:
        ...
        porcupine = pvporcupine.create(
            ...
            sensitivities=[1.0] # <-- FIX v3.31: Set to 1.0
        )
        print("Porcupine wake word engine loaded for 'Vision' (Sensitivity: 1.0).") 
